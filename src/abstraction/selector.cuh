...
struct selector_t{
  ...
  void select(stat_t stats, feature_t& fets, config_t &conf){
    //std::cout << fets.push_workload_ratio << " " << fets.pull_workload_ratio << " " << fets.push_workload << " " << fets.pull_workload << " " << fets.unactive_vertex << " ";
  ...
  void trigger_fusion(stat_t stats, feature_t fets, config_t &conf){
    ...
    if(!conf.conf_fuse_inspect){ // consider to enable fusion
      ...
      // rule 2: for 
      if(fets.growing_rate > 2) conf.conf_idle = 0;
      if(conf.conf_idle >=3){
        conf.conf_fuse_inspect = true; 
        conf.conf_switch_to_fusion = true;
        conf.conf_idle = 0;
  ...
  void fusion(stat_t stats, feature_t fets, config_t &conf){
    ...
    if(conf.conf_dir == Push){
      if(conf.conf_fromall) conf.conf_target = All;
      else conf.conf_target = Active;
    }else{
      if(conf.conf_toall && conf.conf_dir == Pull) conf.conf_target = All;
      else conf.conf_target = Inactive;
    }
  }
  ...
  void select_idem(stat_t& stats, feature_t& fets, config_t &conf){
    ...
    if(fets.push_workload_ratio*5 < fets.pull_workload_ratio) conf.conf_dir = Push;
    //if(fets.push_workload < (1<<18) && fets.push_workload_ratio*2 < fets.pull_workload_ratio) conf.conf_dir = Push;
    

    if(cmd_opt.ins.has_direction) conf.conf_dir = cmd_opt.ins.d;

    // step 2: selection for asfmt
    v1.clear();
    if(conf.conf_dir == Pull){
      conf.conf_asfmt = Bitmap;
      if(fets.fv[12]<0.01) conf.conf_asfmt = Queue;
    }else{
    ...
    if(conf.conf_dir == Pull) conf.conf_lb = TM;
    else{
      v1.clear();
      ...
      if(l==0) conf.conf_lb = WM;
      else if(l==1) conf.conf_lb = CM;
  ...
  void select_ASSO(stat_t& stats, feature_t& fets, config_t &conf){
    ...
    if(conf.conf_fromall && conf.conf_toall){
      ...
      else conf.conf_dir = Push;
      if(cmd_opt.ins.has_direction) conf.conf_dir = cmd_opt.ins.d;
    }

    // step 2: selection for asfmt
    v1.clear();
    if(conf.conf_dir == Push) v1.push_back(fets.fv[11]);
  ...
  void select_based_on_model(stat_t stats, feature_t fets, config_t &conf){TRACE();
    if(fets.centric == EC){
      conf.conf_target = Active;
      if(fets.active_vertex_ratio > 0.8) conf.conf_asfmt = Bitmap;
      else conf.conf_asfmt = Queue;
    ...
    if(fets.pattern==Idem) trigger_fusion(stats, fets, conf);
    else conf.conf_fuse_inspect = conf.conf_fusion;
    //TODO: make it more general
    //if(conf.conf_fuse_inspect) if(fets.max_deg<33 && fets.avg_deg<33 && fets.use_root != -1) conf.conf_super_fusion = true;
    if(conf.conf_fuse_inspect) { fusion(stats, fets, conf); return;}

    if(fets.pattern == Idem) select_idem(stats, fets, conf);
    else if(fets.pattern == ASSO) select_ASSO(stats, fets, conf);
    else if(fets.pattern == Mono) select_Mono(stats, fets, conf);
