...
struct executor_t{
  ...
  void filter(active_set_t &as, G& g, F& f, stat_t& stat, feature_t& fets, config_t& conf){
    ...
    if(conf.conf_fuse_inspect) { 
      ...
      as.get_size_host();
      int workloads = (conf.conf_dir == Push? fets.push_workload:fets.pull_workload);

      conf.ctanum = MIN(CTANUM_EXPAND, workloads/conf.thdnum/2+1);

      if(conf.ctanum < 4) conf.conf_idle +=1;
    ...
    // fortunately, we have used the bitmap to mark the duplicate of the input queue.
    ...
    stat.avg_filter_time = (stat.avg_filter_time*g.level + stat.last_filter_time)/(g.level+1);
  ...
  template<typename G, typename F>
  void expand(active_set_t& as, G& g, F& f, stat_t& stat, feature_t& fets, config_t& conf){TRACE();
    double s = mwtime();
    // TODO: all in one
...
#undef ENABLE
    ...
    stat.avg_expand_time = (stat.avg_expand_time*(g.level-1) + stat.last_expand_time)/g.level;

    fets.last_expand_time = stat.last_expand_time;

    // back to nromal mode
    if(fets.pattern==Idem && fets.avg_expand_time > 0 && fets.last_expand_time > 5*fets.avg_expand_time) {
    ...
    //std::cout << fets.avg_filter_time << " " << fets.avg_expand_time << std::endl;
		if(cmd_opt.verbose){
      stat.show_hints();
      conf.show_hints();
      if(fets.centric == VC) std::cout << "<" << conf.ctanum << "," << conf.thdnum << ">";
      else std::cout << "<" << CTANUM << "," << THDNUM << ">";
      LOG("\n");
		}
