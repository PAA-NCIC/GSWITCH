...
struct chunk_t{
  void build(int nvertexs, int* adj_list, int* start_pos, int* odegrees){
    ...
    std::sort(vvector.begin(), vvector.end(), [](const tup_t& a, const tup_t& b)->bool{return a.id<b.id;});
    ...
    for(const tup_t &it:vvector){cnt[it.id+1]++;}
    for(size_t i=1; i<cnt.size(); ++i){cnt[i] += cnt[i-1];}
    for(int i=0; i<nchunks; ++i) {
      if(cnt[i+1]-cnt[i]>THRESHOLD) heavys.push_back(i);
      else lights.push_back(i);
    }
    for(int i=0; i<nchunks; ++i) ant[i] = ALIGN((cnt[i+1]-cnt[i]), 8);

    dcnt = heavys.size();
    scnt = csize; // row panel;
    LOG(" -- we have %d heavy blocks and %d light block panel\n", dcnt, scnt);
    hid = (int*)malloc(dcnt*sizeof(int));
    for(int i=0; i<dcnt; ++i) hid[i] = heavys[i]/csize;
    ...
    for(int i=0; i<nchunks; ++i){
      if(cnt[i+1]-cnt[i]>THRESHOLD){
        std::sort(&vvector[cnt[i]], &vvector[cnt[i+1]], [](const tup_t& a, const tup_t& b)->bool{if(a.u==b.u) return a.v<b.v; else return a.u<b.u;});
...
#define NID(t,sz) ((t%sz)*THDNUM_EXPAND+(t/sz))
    ...
    for(int i=0; i<dcnt; ++i){
      ...
      //std::cout << chunk_id << " " << cnt[chunk_id] << " " << cnt[chunk_id+1] << std::endl;
      //std::sort(&vvector[cnt[chunk_id]], &vvector[cnt[chunk_id+1]], [](const tup_t& a, const tup_t& b)->bool{return a.u<b.u;});
      for(int j=cnt[chunk_id]; j<cnt[chunk_id+1]; ++j){
        int v = vvector[j].v; // src
        int u = vvector[j].u; // dst
        if(cur!=u){
          segs[base+NID(top,sz)] = HASH(u%BSZ); top++;
          cur = u;
        }
        if(top%sz==0) {segs[base+NID(top,sz)] = HASH(cur%BSZ); top++;}
        segs[base+NID(top,sz)] = v%BSZ; top++;
      }
      if(top > sz*THDNUM_EXPAND){LOG(" -- fill error, exit %d %d %d\n", top, sz*THDNUM_EXPAND, cnt[i+1]-cnt[i]);exit(1); }
    }
#undef NID
    int validcnt = 0;
    for(int i=0; i<dsize; ++i) {if(segs[i]>=0) validcnt++;}
    ...
    for(size_t i = 0; i < lights.size(); ++i){
      ...
      //std::sort(&vvector[cnt[chunk_id]], &vvector[cnt[chunk_id+1]], [](const tup_t& a, const tup_t& b)->bool{if(a.v==b.v) return a.u<b.u; else return a.v<b.v;});
      int panel_id = chunk_id / csize; // row panel
      //ces[panel_id+1] += cnt[chunk_id+1]-cnt[chunk_id];
      ces[panel_id+1] += ant[chunk_id];
    }
    for(int i=1; i<scnt+1; ++i){ces[i] += ces[i-1];} 
    ssize = ces[scnt];
    ...
    if(top != ssize) {LOG(" -- fill sps error\n");exit(1);}

    LOG("step6: build stream buffer\n");
    std::vector<int> pc(nchunks, 0);
    //for(int i=0; i<nchunks; ++i) pc[i] = (cnt[i+1]-cnt[i] >THRESHOLD)? (BSZ):(cnt[i+1]-cnt[i]);
    for(int i=0; i<nchunks; ++i) pc[i] = (cnt[i+1]-cnt[i] >THRESHOLD)? (BSZ):(ant[i]);
    sbsize = 0; for(int i=0; i<nchunks; ++i) sbsize += pc[i];
    ...
    for(int i=1; i<csize+1; ++i) cey[i] += cey[i-1]; 
    if(cey[csize] != sbsize) LOG(" -- cey error!\n");
...
#define CO(i,sz) ((i%sz)*sz+(i/sz))
    ...
    for(int i=0; i<nchunks; ++i) pc_col_acc[i+1]  = pc[CO(i,csize)];
    for(int i=1; i<nchunks+1; ++i) pc_col_acc[i] += pc_col_acc[i-1];

    // spt 
    for(int i=0; i<nchunks; ++i){
      int base = pc_col_acc[CO(i,csize)];
      if(cnt[i+1]-cnt[i] > THRESHOLD){ // heavy block
        for(int j = 0; j < BSZ; ++j) spt[base+j] = j;
    ...
    for(size_t i=0; i<lights.size(); ++i){
      ...
      for(int j=0; j<edge_size; ++j){
        if((j&7) == 0) spf[(sbase+j)>>3] = _base+j;
        //spf[sbase+j] = _base+j;
      }
      sbase += edge_size;
    }
    if(sbase != ssize) LOG(" -- spf error !\n");
...
#undef CO
  ...
  void todevice(chunk_t& c){
    ...
    double gpu_bytes = (dsize+2*dcnt+scnt+2*ssize+2*sbsize+csize+3)*sizeof(int);
