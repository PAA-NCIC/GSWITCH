...
__global__ void __filter_active(active_set_t as){ 
  ...
  for(int idx=gtid; idx<n_bytes; idx+=STRIDE){
    ...
    if(active){ 
      for(int i=0,c=0;i<8;++i) if(active&((char)1<<i)){
...
__global__ void __filter_inactive(active_set_t as){ 
  ...
  for(int idx=gtid; idx<n_bytes; idx+=STRIDE){ 
    ...
    warp_base = __shfl(warp_base, 0); 
    if(inactive){ 
      for(int i=0,c=0;i<8;++i) if(inactive &((char)1<<i)){
        base[warp_base+rank+(c++)] = (idx<<3)+i; 
      } 
    } 
  } 
...
__host__ void __filter_scan(active_set_t &as, config_t conf){ 
  ...
  scan<cta_num, thd_num, int> (as.bins.dg_size,as.bins.dg_offset, CTANUM*THDNUM);
  __compress<<<CTANUM,THDNUM>>> (as.bins.dg_bin, as.bins.dg_size,
         as.bins.dg_offset, Qproxy<M>::output_base(as.queue), Qproxy<M>::output_size(as.queue)); 
}
...
__host__ void 
__filter(active_set_t &as, G g, F f, config_t& conf){TRACE();

#define Launch_Filter(acc, as, conf)                          \
if(conf.conf_qmode==Normal) __filter_##acc<Normal>(as, conf); \
else __filter_##acc<Cached>(as, conf);                        \
  ...
  if(conf.conf_asfmt == Queue){
    bool tofilter = true;
    if(conf.conf_fuse_inspect) tofilter = false;
    ...
    if(tofilter){
      ...
      // if not fused the inspection. there is no need to clean the visited bitmap (only BFS will use it)
