...
struct ExpandProxy<VC,ELB,Push>{
  ...
  static void expand(active_set_t as, device_graph_t<CSR,E> g, F f, config_t conf){
    ...
    if(!conf.conf_inherit){
      ...
      if(as.fmt == Queue){
        if(as.queue.mode == Normal) __ELB_prepare<Queue,Normal><<<1+conf.ctanum/10, conf.thdnum>>>(as, g, conf);
        else __ELB_prepare<Queue,Cached><<<1+conf.ctanum/10, conf.thdnum>>>(as, g, conf);
      }else __ELB_prepare<Bitmap,Normal><<<1+conf.ctanum/10, conf.thdnum>>>(as, g, conf);
