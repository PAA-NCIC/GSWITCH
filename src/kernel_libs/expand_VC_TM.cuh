...
__global__ void
__expand_VC_TM_fused(active_set_t as, G g, F f, config_t conf){
...
template<ASFmt fmt, QueueMode M, typename G, typename F>
__global__ void
__expand_VC_TM(active_set_t as, G g, F f, config_t conf){
  const int* __restrict__ strict_adj_list = g.dg_adj_list;
      ...
      if(w&(1<<lane)){
        ...
        for(int i = start; i < end; ++i){
          ...
          if(toprocess){
            ...
            if(conf.pruning()) break; // idempotent optimization
          }
        }
      }
    }
...
template<ASFmt fmt, QueueMode M, typename G, typename F>
__global__ void 
__rexpand_VC_TM(active_set_t as, G g, F f, config_t conf){
  using edata_t = typename G::edge_t;
  using vdata_t = typename F::wa_t;
  const int* __restrict__ strict_adj_list = g.directed? g.dgr_adj_list : g.dg_adj_list;
  edata_t* strict_edgedata = g.directed? g.dgr_edgedata : g.dg_edgedata;
...
template<>
struct ExpandProxy<VC,TM,Push>{
  template<typename E, typename F>
  static void expand(active_set_t as, device_graph_t<CSR,E> g, F f, config_t conf){
    if(conf.conf_fuse_inspect) {Launch_Expand_VC(TM_fused, as, g, f, conf);}
    else Launch_Expand_VC(TM, as, g, f, conf);
  }
...
template<>
struct ExpandProxy<VC,TM,Pull>{
  template<typename E, typename F>
  static void expand(active_set_t as, device_graph_t<CSR,E> g, F f, config_t conf){
    //if(as.fmt == Bitmap && conf.conf_target == Inactive) // don't know why Bitmap is so slow, thus make it as a special case.
      //__rexpand_VC_TM_BITMAP<<<conf.ctanum, conf.thdnum>>>(as, g, f, conf);
    //else{
      Launch_RExpand_VC(TM, as, g, f, conf);
      //if(as.queue.mode==Normal) __rexpand_VC_TM<Normal><<<conf.ctanum, conf.thdnum>>>(as, g, f, conf);
      //else __rexpand_VC_TM<Cached><<<conf.ctanum, conf.thdnum>>>(as, g, f, conf);
    //}
  }
