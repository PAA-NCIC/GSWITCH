...
struct device_graph_t<COO, E>{
  ...
  int64_t build(host_graph_t<COO,E> hgraph){
    ...
    H_ERR(cudaMemcpy(dg_coo,  hgraph.edge_list,  sizeof(packed_t)*hgraph.nedges, H2D));
...
struct device_graph_t<CSR, E>{
  ...
  int64_t build(host_graph_t<CSR,E> hgraph){
    ...
    if(ENABLE_2D_PARTITION) dg_chunks.todevice(hgraph.chunks);
    ...
    H_ERR(cudaMemcpy(dg_adj_list,  hgraph.adj_list,  sizeof(int)*hgraph.nedges, H2D));
    H_ERR(cudaMemcpy(dg_odegree,   hgraph.odegrees,  sizeof(int)*hgraph.nvertexs, H2D));
    H_ERR(cudaMemcpy(dg_start_pos, hgraph.start_pos, sizeof(int)*hgraph.nvertexs, H2D));
    ...
    if(directed){
      ...
      H_ERR(cudaMemcpy(dgr_adj_list,  hgraph.r_adj_list,  sizeof(int)*hgraph.nedges, H2D));
      H_ERR(cudaMemcpy(dgr_odegree,   hgraph.r_odegrees,  sizeof(int)*hgraph.nvertexs, H2D));
      H_ERR(cudaMemcpy(dgr_start_pos, hgraph.r_start_pos, sizeof(int)*hgraph.nvertexs, H2D));
    }

    if(!std::is_same<Empty,E>::value){
      LOG(" -- edgedata is not Empty\n");
      H_ERR(cudaMalloc((void**)&dg_edgedata,  sizeof(E)*hgraph.nedges)); 
      H_ERR(cudaMemcpy(dg_edgedata, hgraph.edge_weights, sizeof(int)*hgraph.nedges, H2D));

      if(directed){
        H_ERR(cudaMalloc((void**)&dgr_edgedata,  sizeof(E)*hgraph.nedges)); 
        H_ERR(cudaMemcpy(dgr_edgedata, hgraph.r_edge_weights, sizeof(int)*hgraph.nedges, H2D));
      }
      //if(std::is_same<float,E>::value) build_tex<float>(dt_edgedata, (float*)dg_edgedata, nedges);
  ...
  __device__ __tbdinline__
  E* fetch_edata(const int eid){return dg_edgedata+eid;}
  ...
  int get_in_degree(const int vid){
    if(directed) return tex1Dfetch<int>(dtr_odegree, vid);
    return tex1Dfetch<int>(dt_odegree, vid); 
  }
...
struct Fake<VC,E>{ 
  ...
  static graph_t<CSR,E>
  func2(edgelist_t<E> el, feature_t& fets){
    graph_t<CSR,E> g;
    g.el = el;
    g.hg.build(g.el);
...
auto build_graph(std::string path, feature_t &fets, bool with_header=false, bool with_weight=false, bool directed=false)
  ->decltype(Fake<C,E>::func(path,fets,with_header,with_weight,directed)){
    return Fake<C,E>::func(path,fets,with_header,with_weight,directed);
}

template<Centric C, typename E=Empty>
auto build_graph(edgelist_t<E> el, feature_t& fets)
  ->decltype(Fake<C,E>::func2(el, fets)){
