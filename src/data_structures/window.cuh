...
struct window_t{
  ...
  void adjust(feature_t fets, config_t conf){
    if(conf.conf_fusion) { wsz = wsz_lim; return; };
  ...
  void inflat(){wsz = wsz/inflat_rate; if(wsz > wsz_lim) wsz = wsz_lim;}

  __device__ __tbdinline__ 
  bool in(float x){if(!enable) return true; else return x<th+wsz && x>=th;}

  __device__ __tbdinline__ 
  bool less(float x){return x<th;}

  __device__ __tbdinline__ 
  bool greater(float x){return x>=th+wsz; }
