...
struct SSSP : Functor<VC, int, int, int> {
  ...
  __device__ int emit(int vid, int *w, G g) { return *wa_of(vid) + *w; }

  __device__ bool comp(int *disu, int newv, G g) {
    if (*disu > newv) {
      *disu = newv;
      return data.window.in(newv);
    }
    return false;
  }
  ...
  __device__ bool exit(int v, G g) { return *wa_of(v) != *ra_of(v); }
};
...
template <typename G, typename F> double run_sssp(G g, F f, int root) {
  // step 1: initializing
  LOG(" -- Initializing\n");
...
int main(int argc, char *argv[]) {
  ...
  edgelist_t<int> el;
  el.read_mtx(cmd_opt.path, cmd_opt.directed, cmd_opt.with_weight,
              cmd_opt.with_header);
  el.gen_weight(64);
